...
- Added 18 new tools
- Added passive subdomains discovery tools: chaos, sonarbyte, crobat, ctfr, crtsh, waybackurls, gau, github-subdomains,  gitlab-subdomains, Subdomainizer, cero
- Added Subdomains resolving using dnsx to allow port scanning on all resolvable subdomains  
- Added subdomains brute force tools: dnsx, aiodnsbrute, shuffledns amd puredns.
- Added 'alive_only' configuration in scan engine yaml for inserting only resolved subdomains to db.
- Added 'filter_wildcard' configuration in scan engine yaml (uses puredns) 
- Better validation and sorting subdomains discovery tools output using filter_subs.py and some bash magic
- Added checks to determine if discovered subdomain is matching the TLD domain to filter unrelated subdomains using 'tldend'.
- Added resolvers support to most tools and added resolvers configuration in 'Tool Settings'.
- Added dnsvalidator tool for validating resolvers and config section in scan engine yaml 'validate_resolvers'.
- Added configuration support in 'Tool Settings' for both subfinder config and providers config and updated subfinder syntax.
- Added cent tool for downloading nuclei templates.
- Added cutom nuclei templates from github
- Added option to fingerprint all opened ports on a subdomains using httpx.
- Added options to scan all opened ports on a subdomains using nuclei.
- Added custom default scan engines.
- Changed UI count for subdomains and endpoints to count only unique results.
- Changed 'Alive subdomains' count to count only subdomains that has been resolved to an an IP Address ip address instead of http response code.
- And more...?
