...
// get Subfinder providers
$.getJSON(`/api/getFileContents?subfinder_providers&format=json`, function(data) {
  $("#subfinder_providers_text_area").attr("rows", 14);
  $("textarea#subfinder_providers_text_area").html(htmlEncode(data['content']));
}).fail(function(){
  $("#subfinder_providers_text_area").removeAttr("readonly");
  $("textarea#subfinder_providers_text_area").html(`# Your Subfinder providers configuration here.`);
  $("#subfinder-providers-form").append('<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="subfinder-providers-submit">');
});

$("#subfinder_providers_text_area").dblclick(function() {
  if (!document.getElementById('subfinder-providers-submit')) {
    $("#subfinder_providers_text_area").removeAttr("readonly");
    $("#subfinder-providers-form").append('<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="subfinder-providers-submit">');
  }
});
...
// get dns resolvers config
$.getJSON(`/api/getFileContents?resolvers_config&format=json`, function(data) {
  $("#resolvers_config_text_area").attr("rows", 14);
  $("textarea#resolvers_config_text_area").html(htmlEncode(data['content']));
}).fail(function(){
  $("#resolvers_config_text_area").removeAttr("readonly");
  $("textarea#resolvers_config_text_area").html(`# Your resolvers list goes here.`);
  $("#resolvers-config-form").append('<input type="submit" class="btn btn-primary mt-2 float-right" value="Save Changes" id="resolvers-config-submit">');
});

$("#resolvers_config_text_area").dblclick(function() {
  if (!document.getElementById('resolvers-config-submit')) {
    $("#resolvers_config_text_area").removeAttr("readonly");
    $("#resolvers-config-form").append('<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="resolvers-config-submit">');
  }
});

// get trusted dns resolvers config
$.getJSON(`/api/getFileContents?trusted_resolvers_config&format=json`, function(data) {
  $("#trusted_resolvers_config_text_area").attr("rows", 14);
  $("textarea#trusted_resolvers_config_text_area").html(htmlEncode(data['content']));
}).fail(function(){
  $("#trusted_resolvers_config_text_area").removeAttr("readonly");
  $("textarea#trusted_resolvers_config_text_area").html(`# Your trusted resolvers list goes here.`);
  $("#trusted-resolvers-config-form").append('<input type="submit" class="btn btn-primary mt-2 float-right" value="Save Changes" id="trusted-resolvers-config-submit">');
});

$("#trusted_resolvers_config_text_area").dblclick(function() {
  if (!document.getElementById('trusted-resolvers-config-submit')) {
    $("#trusted_resolvers_config_text_area").removeAttr("readonly");
    $("#trusted-resolvers-config-form").append('<input type="submit" class="btn btn-primary mt-2 float-end" value="Save Changes" id="trusted-resolvers-config-submit">');
  }
});