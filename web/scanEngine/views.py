...
def tool_specific_settings(request):
    ...
    if request.method == "POST":
        ...
        elif 'subfinder_providers_text_area' in request.POST:
            with open('/root/.config/subfinder/provider-config.yaml', "w") as fhandle:
                fhandle.write(request.POST.get('subfinder_providers_text_area'))
            messages.add_message(request, messages.INFO, 'Subfinder providers config updated!')
            return http.HttpResponseRedirect(reverse('tool_settings'))
        ...
        elif 'resolvers_config_text_area' in request.POST:
            with open('/root/.config/resolvers.txt', "w") as fhandle:
                fhandle.write(request.POST.get('resolvers_config_text_area'))
            messages.add_message(request, messages.INFO, 'resolvers list updated!')
            return http.HttpResponseRedirect(reverse('tool_settings'))
        
        elif 'trusted_resolvers_config_text_area' in request.POST:
            with open('/root/.config/trusted_resolvers.txt', "w") as fhandle:
                fhandle.write(request.POST.get('trusted_resolvers_config_text_area'))
            messages.add_message(request, messages.INFO, 'Trusted resolvers list updated!')
            return http.HttpResponseRedirect(reverse('tool_settings'))
