...
class GetFileContents(APIView):
	def get(self, request, format=None):
		...
		if 'subfinder_providers' in req.query_params:
			path = "/root/.config/subfinder/provider-config.yaml"
			if not os.path.exists(path):
				os.system('touch {}'.format(path))
				response['message'] = 'File Created!'
			f = open(path, "r")
		...
		if 'resolvers_config' in req.query_params:
			path = "/root/.config/resolvers.txt"
			if not os.path.exists(path):
				os.system('touch {}'.format(path))
				response['message'] = 'File Created!'
			f = open(path, "r")
		...
		if 'trusted_resolvers_config' in req.query_params:
			path = "/root/.config/trusted_resolvers.txt"
			if not os.path.exists(path):
				os.system('touch {}'.format(path))
				response['message'] = 'File Created!'
			f = open(path, "r")
