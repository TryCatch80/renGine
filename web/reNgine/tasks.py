...
def initiate_scan(
	...
	if yaml_configuration[SUBDOMAIN_DISCOVERY][VALIDATE_RESOLVERS] == True:
		update_last_activity(activity_id, 2)
		activity_id = create_scan_activity(task, "DNS Resolver Validation", 1)
		dns_validator(
			task,
			activity_id,
			yaml_configuration,
	...
	if(task.subdomain_discovery):
		...
		logger.info("[*] Dumping current subdomains from db")
		subdomain_db_results_file = results_dir + '/from_db.txt'
		with open(subdomain_db_results_file, 'w+') as file:
			subdomains = Subdomain.objects.filter(target_domain__id=domain_id)
			for d in subdomains:
				file.write(d.name + '\n')
			file.close()

		skip_subdomain_scan(task, domain, yaml_configuration, results_dir)

	update_last_activity(activity_id, 2)
	activity_id = create_scan_activity(task, "DNS Resolver", 1)
	dns_resolver(
		task,
		domain,
  		yaml_configuration,
...
def skip_subdomain_scan(task, domain, yaml_configuration, results_dir):
	...
	os.system(
		'cat {0}/target_domain.txt > {0}/unsorted_subdomain_collection.txt'.format(results_dir))

	os.system(
		'cat {0}/from_*.txt >> {0}/unsorted_subdomain_collection.txt'.format(results_dir))
 
	os.system(
		'cat {}/unsorted_subdomain_collection.txt | python3 /usr/src/app/tools/filter_subs.py | tr [:upper:] [:lower:] | tldend -d {} > {}/subdomain_collection.txt'.format(results_dir, domain.name, results_dir))

	os.system('rm -f {}/from_*.txt'.format(results_dir))
	...
	os.system('rm -f {}/unsorted_subdomain_collection.txt'.format(results_dir))
	...
	if yaml_configuration[SUBDOMAIN_DISCOVERY][VALIDATE_RESOLVERS] == True and os.path.isfile('{}/valid_resolvers.txt'.format(results_dir)) == True:
		dns_resolvers = "{}/valid_resolvers.txt".format(results_dir)
	else:
		dns_resolvers = "/root/.config/resolvers.txt"
  
	if yaml_configuration[SUBDOMAIN_DISCOVERY][ALIVE_ONLY] == True:
		logger.info("[+] Alive Only Scan: True")
		logger.info("[*] Checking for alive subdomains and filtering wildcards")
		if yaml_configuration[SUBDOMAIN_DISCOVERY][FILTER_WILDCARD] == True:
			#check_alive_command = 'dnsx -l {}/sorted_subdomain_collection.txt -r {} -retry 5 -stats -v -o {}/sorted_subdomain_collection_alive.txt -wd {} -t {}'.format(results_dir, dns_resolvers, results_dir, domain.name, threads)
			check_alive_command = 'puredns resolve {}/sorted_subdomain_collection.txt -r {} --resolvers-trusted /root/.config/trusted_resolvers.txt --wildcard-tests 50 --rate-limit 8000 --write {}/sorted_subdomain_collection_alive.txt'.format(results_dir, dns_resolvers, results_dir)
		else:
			#check_alive_command = 'dnsx -l {}/sorted_subdomain_collection.txt -r {} -retry 5 -stats -v -o {}/sorted_subdomain_collection_alive.txt -t {}'.format(results_dir, dns_resolvers, results_dir, threads)
			check_alive_command = 'puredns resolve {}/sorted_subdomain_collection.txt -r {} --resolvers-trusted /root/.config/trusted_resolvers.txt --skip-wildcard-filter --rate-limit 8000 --write {}/sorted_subdomain_collection_alive.txt'.format(results_dir, dns_resolvers, results_dir)
		
   
		logging.info(check_alive_command)	
		process = subprocess.Popen(check_alive_command.split())
		process.wait()
		subdomain_scan_results_file = results_dir + '/sorted_subdomain_collection_alive.txt'
	else:
		logger.info("[+] Alive Only Scan: False")
		subdomain_scan_results_file = results_dir + '/sorted_subdomain_collection.txt'

	subdomain_db_results_file = subdomain_scan_results_file
	logger.info("[*] Importing db results back to db")
	with open(subdomain_db_results_file) as subdomain_list:
			for _subdomain in subdomain_list:
				...
				#if not Subdomain.objects.filter(name=__subdomain).exists(
...
def subdomain_scan(
	...
	logger.info("[*] Dumping db results to file")
	subdomain_db_results_file = results_dir + '/from_db.txt'
	with open(subdomain_db_results_file, 'w+') as file:
		subdomains = Subdomain.objects.filter(target_domain__id=domain_id)
		for d in subdomains:
	...
	if ALL in yaml_configuration[SUBDOMAIN_DISCOVERY][USES_TOOLS]:
		tools = 'amass-active amass-passive assetfinder sublist3r subfinder oneforall chaos sonarbyte crobat ctfr crtsh waywayback gau github-subdomains gitlab-subdomains subdomainizer dnsx aiodnsbrute shuffledns puredns cero subress crawlers'
	...
	try:
		for tool in tools.split(' '):
			if tool in tools.split(' '):
				if tool == 'amass-passive':
					amass_command = 'amass enum -passive -rf {} -d {} -o {}/from_amass.txt'.format(
							dns_resolvers, domain.name, results_dir)
					...
					amass_command = 'amass enum -active -rf {} -d {} -o {}/from_amass_active.txt'.format(
							dns_resolvers, domain.name, results_dir)
					...
					assetfinder_command = 'assetfinder --subs-only {} | tee {}/from_assetfinder.txt'.format(
					...
					subfinder_command = 'subfinder -all -rL {} -nW -d {} -t {} -o {}/from_subfinder.txt'.format(
						dns_resolvers, domain.name, threads, results_dir)
					...
					if USE_SUBFINDER_CONFIG in yaml_configuration[SUBDOMAIN_DISCOVERY] and yaml_configuration[SUBDOMAIN_DISCOVERY][USE_SUBFINDER_CONFIG]:
						subfinder_command += ' -config /root/.config/subfinder/config.yaml -pc /root/.config/subfinder/provider-config.yaml'
					...
					chaos_token = yaml_configuration[SUBDOMAIN_DISCOVERY][CHAOS_API]
					chaos_command = 'chaos -d {} -key {} -o {}/from_chaos.txt'.format(domain.name, chaos_token, results_dir)
					# Run chaos
					logging.info(chaos_command)
					process = subprocess.Popen(chaos_command.split())
					process.wait()

				elif tool == 'dnsx':
					if DNSX_WORDLIST in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						wordlist = yaml_configuration[SUBDOMAIN_DISCOVERY][DNSX_WORDLIST]
						if wordlist == 'default':
							wordlist_path = '/usr/src/wordlist/deepmagic.com-prefixes-top50000.txt'
						else:
							wordlist_path = '/usr/src/wordlist/' + wordlist + '.txt'
							if not os.path.exists(wordlist_path):
								wordlist_path = '/usr/src/' + DNSX_WORDLIST
						logging.info("[*] Bruteforcing Subdomains using dnsx")

						if yaml_configuration[SUBDOMAIN_DISCOVERY][FILTER_WILDCARD] == True:
							dnsx_brute_command = 'dnsx -d {} -w {} -r {} -retry 5 -stats -v -o {}/from_dnsx_brute.txt -wd {} -t {}'.format(domain.name, wordlist_path, dns_resolvers, results_dir, domain.name, threads)
						else:
							dnsx_brute_command = 'dnsx -d {} -w {} -r {} -retry 5 -stats -v -o {}/from_dnsx_brute.txt -t {}'.format(domain.name, wordlist_path, dns_resolvers, results_dir, threads)

						logging.info(dnsx_brute_command)
						process = subprocess.Popen(dnsx_brute_command.split())
						process.wait()

				elif tool == 'aiodnsbrute':
					if AIODNSBRUTE_THREADS in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						aio_threads = yaml_configuration[SUBDOMAIN_DISCOVERY][AIODNSBRUTE_THREADS]
					else:
						aio_threads = yaml_configuration[SUBDOMAIN_DISCOVERY][THREADS]
					if AIODNSBRUTE_WORDLIST in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						wordlist = yaml_configuration[SUBDOMAIN_DISCOVERY][AIODNSBRUTE_WORDLIST]
						if wordlist == 'default':
							wordlist_path = '/usr/src/wordlist/deepmagic.com-prefixes-top50000.txt'
						else:
							wordlist_path = '/usr/src/wordlist/' + wordlist + '.txt'
							if not os.path.exists(wordlist_path):
								wordlist_path = '/usr/src/' + AIODNSBRUTE_WORDLIST
						logging.info("[*] Bruteforcing Subdomains using aiodnsbrute")

						if yaml_configuration[SUBDOMAIN_DISCOVERY][FILTER_WILDCARD] == True:
							aiodnsbrute_command = 'aiodnsbrute -r {} -w {} -t {} -f {}/aiodnsbrute.csv -o csv --wildcard {}'.format(dns_resolvers, wordlist_path, aio_threads, results_dir, domain.name)
						else:
							aiodnsbrute_command = 'aiodnsbrute -r {} -w {} -t {} -f {}/aiodnsbrute.csv -o csv --no-wildcard {}'.format(dns_resolvers, wordlist_path, aio_threads, results_dir, domain.name)

						logging.info(aiodnsbrute_command)
						process = subprocess.Popen(aiodnsbrute_command.split())
						process.wait()

						os.system("cat {0}/aiodnsbrute.csv | cut -d ',' -f1 | grep -v 'Hostname' > {0}/from_aiodnsbrute.txt".format(results_dir))
						os.system("rm {0}/aiodnsbrute.csv".format(results_dir))
      
				elif tool == 'puredns':
					if PUREDNS_RATELIMIT in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						puredns_ratelimit = yaml_configuration[SUBDOMAIN_DISCOVERY][PUREDNS_RATELIMIT]
					else:
						puredns_ratelimit = '8000'
					
					if PUREDNS_WORDLIST in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						wordlist = yaml_configuration[SUBDOMAIN_DISCOVERY][PUREDNS_WORDLIST]
						if wordlist == 'default':
							wordlist_path = '/usr/src/wordlist/deepmagic.com-prefixes-top50000.txt'
						else:
							wordlist_path = '/usr/src/wordlist/' + wordlist + '.txt'
							if not os.path.exists(wordlist_path):
								wordlist_path = '/usr/src/' + PUREDNS_WORDLIST
						
						if yaml_configuration[SUBDOMAIN_DISCOVERY][GENERATE_CUSTOM_WORDLIST] == True:
							logging.info('Generating custom wordlist')
							str_replace_command = "cat {0}/from_*.txt | sort -u | str-replace -d '.' -n | sort -u >> {0}/tmp_wordlist.txt".format(results_dir)
							logging.info(str_replace_command)
							os.system(str_replace_command)
							logging.info('Appending wordlist')
							append_command = "cat {0} >> {1}/tmp_wordlist.txt".format(wordlist_path, results_dir)
							logging.info(append_command)
							os.system(append_command)
							logging.info('Sorting wordlist...')
							sotring_command = "cat {0}/tmp_wordlist.txt | sort -u > {0}/custom_wordlist.txt".format(results_dir)
							logging.info(sotring_command)
							os.system(sotring_command)
							logging.info('Removing temp files...')
							cleaning_command = "rm {0}/tmp_wordlist.txt".format(results_dir)
							logging.info(cleaning_command)
							os.system(cleaning_command)
							wordlist_path = '{0}/custom_wordlist.txt'.format(results_dir)
							logging.info('Custom wordlist created custom_wordlist.txt'.format(results_dir))
       

						if yaml_configuration[SUBDOMAIN_DISCOVERY][FILTER_WILDCARD] == True:
							puredns_command = 'puredns bruteforce {} {} -r {} --resolvers-trusted /root/.config/trusted_resolvers.txt --wildcard-tests 50 --rate-limit {} --write {}/from_puredns.txt'.format(wordlist_path, domain.name, dns_resolvers, puredns_ratelimit, results_dir)
						else:
							puredns_command = 'puredns bruteforce {} {} -r {} --resolvers-trusted /root/.config/trusted_resolvers.txt --rate-limit {} --skip-wildcard-filter --write {}/from_puredns.txt'.format(wordlist_path, domain.name, dns_resolvers, puredns_ratelimit, results_dir)
						
						logging.info("[*] Bruteforcing Subdomains using puredns")
						logging.info(puredns_command)
						process = subprocess.Popen(puredns_command.split())
						process.wait()

						os.system('rm {0}/custom_wordlist.txt'.format(results_dir))

						if yaml_configuration[SUBDOMAIN_DISCOVERY][GENERATE_PERMUTATIONS] == True:
							
							if ALL in yaml_configuration[SUBDOMAIN_DISCOVERY][PERMUTATION_TOOLS]:
									perm_tools = 'str-replace mksub ripgen goaltdns gotator'
							else:
								perm_tools = ' '.join(
									str(perm_tool) for perm_tool in yaml_configuration[SUBDOMAIN_DISCOVERY][PERMUTATION_TOOLS])

							permutation_level = yaml_configuration[SUBDOMAIN_DISCOVERY][PERMUTATION_LEVEL]
							
							logging.info('Generating permutations')
							logging.info('Gathering and sorting all found subdomains...')
							gather_command = "cat {0}/from_*.txt | sort -u | tldend -d {1} | sort -u > {0}/tmp_subs.txt".format(results_dir, domain.name)
							logging.info(gather_command)
							os.system(gather_command)
							for perm_tool in perm_tools.split(' '):
								if perm_tool == 'str-replace':
									logging.info('Generating permutations using str-replace...')
									str_perm_command = "cat {0}/tmp_subs.txt | str-replace -W /usr/src/app/tools/wordlists/altdns.txt -tld {1} >> {0}/tmp_permutations.txt".format(results_dir, domain.name)
									logging.info(str_perm_command)
									os.system(str_perm_command)
								elif perm_tool == 'mksub':
									logging.info('Generating permutations using mksub...')
									str_perm_command = "mksub -d {0} -w /usr/src/app/tools/wordlists/mksub.txt -l {1} -t 500 >> {2}/tmp_permutations.txt".format(domain.name, permutation_level, results_dir)
									logging.info(str_perm_command)
									os.system(str_perm_command)
								elif perm_tool == 'ripgen':
									logging.info('Generating permutations using ripgen...')
									ripgen_command = "cat {0}/tmp_subs.txt | ripgen -w /usr/src/app/tools/wordlists/altdns.txt >> {0}/tmp_permutations.txt".format(results_dir)
									logging.info(ripgen_command)
									os.system(ripgen_command)
								elif perm_tool == 'goaltdns':
									logging.info('Generating permutations using goaltdns...')
									goaltdns_command = "cat {0}/tmp_subs.txt | goaltdns -w /usr/src/app/tools/wordlists/altdns.txt >> {0}/tmp_permutations.txt".format(results_dir)
									logging.info(goaltdns_command)
									os.system(goaltdns_command)
								elif perm_tool == 'gotator':		
									logging.info('Generating permutations using gotator...')
									gotator_command = "gotator -sub {0}/tmp_subs.txt -perm /usr/src/app/tools/wordlists/altdns.txt -depth {1} -numbers 10 -mindup -adv -md -t 2000 -silent >> {0}/tmp_permutations.txt".format(results_dir, permutation_level)
									logging.info(gotator_command)
									os.system(gotator_command)
							
							logging.info('Sorting permutation list...')
							perm_sort_command = "cat {0}/tmp_permutations.txt | sort -u > {0}/permutations_to_resolve.txt".format(results_dir)
							logging.info(perm_sort_command)
							os.system(perm_sort_command)
							logging.info('Removing temp files...')
							os.system("rm {0}/tmp_permutations.txt".format(results_dir))
							os.system("rm {0}/tmp_subs.txt".format(results_dir))
							permurations_command = "puredns resolve {0}/permutations_to_resolve.txt -r {1} --resolvers-trusted /root/.config/trusted_resolvers.txt --write {0}/from_permutations.txt --wildcard-tests 50 --rate-limit {2}".format(results_dir, dns_resolvers, puredns_ratelimit)
							logging.info('Resolving permutations...')
							logging.info(permurations_command)
							os.system(permurations_command)
							logging.info('Removing temp files...')
							os.system('rm {0}/permutations_to_resolve.txt'.format(results_dir))
							
				elif tool == 'subdomainizer':
					subdomainizer_command = 'python3 /usr/src/github/SubDomainizer/SubDomainizer.py -u {} -san same -k -o {}/from_subdomainizer.txt'.format(domain.name, results_dir)
					logging.info(subdomainizer_command)
					process = subprocess.Popen(subdomainizer_command.split())
					process.wait()

				elif tool == 'sonarbyte':
					sonarbyte_command = 'python3 /usr/src/app/tools/Sonarbyte.py -d {} -o {}/from_sonarbyte.txt'.format(domain.name, results_dir)
					logging.info(sonarbyte_command)
					process = subprocess.Popen(sonarbyte_command.split())
					process.wait()

				elif tool == 'crobat':
					crobat_command = 'crobat -s {} -u | tee {}/from_crobat.txt'.format(domain.name, results_dir)
					logging.info(crobat_command)
					#process = subprocess.Popen(crobat_command.split())
					#process.wait()
					os.system(crobat_command)

				elif tool == 'ctfr':
					ctfr_command = 'python3 /usr/src/app/tools/ctfr.py -d {} | tee {}/from_ctfr.txt'.format(domain.name, results_dir)
					logging.info(ctfr_command)
					#process = subprocess.Popen(ctfr_command.split())
					#process.wait()
					os.system(ctfr_command)

				elif tool == 'crtsh':
					crtsh_command = 'python3 /usr/src/app/tools/crt-sh.py -d {} -r | tee {}/from_crtsh.txt'.format(domain.name, results_dir)
					logging.info(crtsh_command)
					#process = subprocess.Popen(crtsh_command.split())
					#process.wait()
					os.system(crtsh_command)

				elif tool == 'waywayback':
					waywayback_command = "echo {} | waybackurls | unfurl -u domains | tee {}/from_waywayback.txt".format(domain.name, results_dir)
					logging.info(waywayback_command)
					#process = subprocess.Popen(waywayback_command.split())
					#process.wait()
					os.system(waywayback_command)

				elif tool == 'gau':
					logging.info("Executing gau...")
					gau_command = "echo {} | gau --subs --threads {} | unfurl -u domains | tee {}/from_gau.txt".format(domain.name, threads, results_dir)
					logging.info(gau_command)
					#process = subprocess.Popen(gau_command.split())
					#process.wait()
					os.system(gau_command)
					logging.info("Executing gauplus...")
					gau_plus_command = " echo {} | gauplus -t {} -subs -random-agent -b eot,jpeg,gif,css,tif,tiff,otf,woff2,ico,pdf,txt,js,doc,docx,ttf,woff,svg,png,jpg,mp4,xls,xlsx,ppt,pptx,bmp | unfurl -u domains | tee {}/from_gauplus.txt".format(domain.name, threads, results_dir)
					logging.info(gau_plus_command)
					os.system(gau_plus_command)

				elif tool == 'github-subdomains':
					github_token = yaml_configuration[SUBDOMAIN_DISCOVERY][GITHUB_API]
					gitsubs_command = "github-subdomains -d {} -t {} -o {}/from_gitsubs.txt".format(domain.name, github_token, results_dir)
					logging.info(gitsubs_command)
					#process = subprocess.Popen(gitsubs_command.split())
					#process.wait()
					os.system(gitsubs_command)
     
				elif tool == 'gitlab-subdomains':
					gitlab_token = yaml_configuration[SUBDOMAIN_DISCOVERY][GITLAB_API]
					gitlab_command = "gitlab-subdomains -d {} -t {} | tee {}/from_gitlab.txt".format(domain.name, gitlab_token, results_dir)
					logging.info(gitlab_command)
					#process = subprocess.Popen(gitsubs_command.split())
					#process.wait()
					os.system(gitlab_command)
					...
					if SHFFLEDNS_THREADS in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						shuffle_threads = yaml_configuration[SUBDOMAIN_DISCOVERY][SHFFLEDNS_THREADS]
					else:
						shuffle_threads = yaml_configuration[SUBDOMAIN_DISCOVERY][THREADS]
					if SHFFLEDNS_WORDLIST in yaml_configuration[SUBDOMAIN_DISCOVERY]:
						wordlist = yaml_configuration[SUBDOMAIN_DISCOVERY][SHFFLEDNS_WORDLIST]
						if wordlist == 'default':
							wordlist_path = '/usr/src/wordlist/deepmagic.com-prefixes-top50000.txt'
						else:
							wordlist_path = '/usr/src/wordlist/' + wordlist + '.txt'
							if not os.path.exists(wordlist_path):
								wordlist_path = '/usr/src/' + SHFFLEDNS_WORDLIST
        
					shuffledns_command = "shuffledns -d {} -w {} -r {} -sw -o {}/from_shuffledns.txt -t {}".format(domain.name, wordlist_path, dns_resolvers, results_dir, shuffle_threads)
					logging.info(shuffledns_command)
					os.system(shuffledns_command)
     
				elif tool == 'cero':
					cero_command = "cat {}/from_*.txt | sort -u | cero -d -c {} | tldend -d {} | sort -u | tee {}/from_cero.txt".format(results_dir, threads, domain.name, results_dir)
					logging.info(cero_command)
					os.system(cero_command)
     
				elif tool == 'subress':
					subress_command = "cat {0}/from_*.txt | sort -u | subress -d {1} -q -t {2} | tldend -d {2} | sort -u | tee {0}/from_subress.txt".format(results_dir, domain.name, threads)
					logging.info(subress_command)
					os.system(subress_command)
	
				elif tool == 'crawlers':
					check_http = 'cat {0}/from_*.txt | sort -u | tldend -d {1} | sort -u | /go/bin/httpx -t {2} -random-agent -r {3} -retries 3 -timeout 10 -silent | tee {0}/tmp_crawlers.txt'.format(results_dir, domain.name, threads, dns_resolvers)
					logging.info('Generating URLs for subdomain discovery from endpoints using each discovered subdomain so far...')
					logging.info(check_http)
					os.system(check_http)
					
					gospider_subs_command = "gospider -S {0}/tmp_crawlers.txt --js -t {1} -d 2 --sitemap --robots -w -r | grep -Eo 'https?://([a-z0-9]+[.])*{2}.*' | unfurl -u domain | tee {0}/from_gospider.txt".format(results_dir, threads, domain.name)
					logging.info('execute gospider to find subdomains...')
					logging.info(gospider_subs_command)
					os.system(gospider_subs_command)
     
					gauplus_subs_command = "cat {0}/tmp_crawlers.txt | gauplus -t {1} -subs -random-agent -b eot,jpeg,gif,css,tif,tiff,otf,woff2,ico,pdf,txt,js,doc,docx,ttf,woff,svg,png,jpg,mp4,xls,xlsx,ppt,pptx,bmp | grep -Eo 'https?://([a-z0-9]+[.])*{2}.*' | unfurl -u domains | tee {0}/from_gauplus_deep.txt".format(results_dir, threads, domain.name)
					logging.info('execute gauplus to find subdomains...')
					logging.info(gauplus_subs_command)
					os.system(gauplus_subs_command)
					
					hakrawler_subs_command = "cat {0}/tmp_crawlers.txt | hakrawler -subs -u | grep -Eo 'https?://([a-z0-9]+[.])*{1}.*' | unfurl -u domains | tee {0}/from_hakrawler_deep.txt".format(results_dir, domain.name)
					logging.info('execute hakrawler to find subdomains...')
					logging.info(hakrawler_subs_command)
					os.system(hakrawler_subs_command)
					
					#waybackurls_subs_command = "cat {0}/tmp_crawlers.txt | waybackurls | grep -Eo 'https?://([a-z0-9]+[.])*{1}.*' | unfurl -u domains | tee {0}/from_waybackurls_deep.txt".format(results_dir,domain.name)
					#logging.info('execute wybackurls to find subdomains...')
					#logging.info(waybackurls_subs_command)
					#os.system(waybackurls_subs_command)
    
    
					os.system('rm {}/tmp_crawlers.txt'.format(results_dir))
...
def dns_resolver(task, domain, yaml_configuration, results_dir, activity_id):
	...
	notification = Notification.objects.all()
	if notification and notification[0].send_scan_status_notif:
		send_notification('DNS Resolving for target {} has been initiated.'.format(domain.name))

	dnsx_results_file = results_dir + '/dnsx.json'
	...
	dnsx_command = 'dnsx -l {} -a -retry 5 -r {} -json -o {}'.format(subdomain_scan_results_file, dns_resolvers, dnsx_results_file)

	logger.info(dnsx_command)
	os.system(dnsx_command)
	
	try:
		dnsx_json_result = open(dnsx_results_file, 'r')
		lines = dnsx_json_result.readlines()
		for line in lines:
...
def dns_validator(task, activity_id, yaml_configuration, results_dir):
    
	threads = 50
	if DNSVALIDATOR_THREADS in yaml_configuration[SUBDOMAIN_DISCOVERY]:
		_threads = yaml_configuration[SUBDOMAIN_DISCOVERY][DNSVALIDATOR_THREADS]
		if _threads > 0:
			threads = _threads
	'''
	if os.path.isfile('/root/.config/valid_resolvers.txt') == True:
		os.system('rm /root/.config/valid_resolvers.txt')
	'''
	dnsvalidator_command = "dnsvalidator -tL /root/.config/resolvers.txt -threads {} -o {}/valid_resolvers.txt".format(threads, results_dir)
	logger.info(dnsvalidator_command)
	os.system(dnsvalidator_command)
...
def http_crawler(task, domain, yaml_configuration, results_dir, activity_id, threads=100):
	...
	if ALL_PORTS in yaml_configuration[VULNERABILITY_SCAN] and yaml_configuration[VULNERABILITY_SCAN][ALL_PORTS] == True:
    		
		check_open_ports = 'naabu -r {} -list {} -exclude-cdn -o {}/subdomain_ports.txt'.format(dns_resolvers, subdomain_scan_results_file, results_dir)
		if PORTS in yaml_configuration[PORT_SCAN]:
    		# TODO:  legacy code, remove top-100 in future versions
			scan_ports = yaml_configuration[PORT_SCAN][PORTS]
		if 'full' in scan_ports:
			check_open_ports += ' -p -'
		elif 'top-100' in scan_ports:
			check_open_ports += ' -top-ports 100 '
		elif 'top-1000' in scan_ports:
			check_open_ports += ' -top-ports 1000 '
		else:
			scan_ports = ','.join(
				str(port) for port in scan_ports)
			check_open_ports += ' -p {} '.format(scan_ports)
		logger.info(check_open_ports)
		os.system(check_open_ports)
		subdomain_scan_results_file = '{}/subdomain_ports.txt'.format(results_dir)
		

	httpx_command = '/go/bin/httpx -status-code -content-length -title -tech-detect -cdn -ip -follow-host-redirects -random-agent -r {} -t {} -retries 3 -timeout 10'.format(dns_resolvers, threads)
		...
		for line in lines:
			...
			try:
				...
				if 'url' in json_st:
					...
					scan_history=task, name=json_st['input'].split(":")[0])
				else:
					subdomain = Subdomain.objects.get(
						scan_history=task, name=json_st['url'].split("//")[-1].split(":")[0])
				'''
				...
				subdomain.discovered_dfate = discovered_date
...
def port_scanning(
	...
	if domain:
		...
		naabu_command = 'naabu -r {} -list {} -json -o {}'.format(
			dns_resolvers,
   			subdomain_scan_results_file,
			port_results_file
		)
	elif subdomain:
		naabu_command = 'naabu -r {} -host {} -o {} -json '.format(
			dns_resolvers,
   			subdomain,
			port_results_file
		)
	...
	if os.path.isfile(port_results_file) == False:
		os.system('touch ' + port_results_file)
...
def fetch_endpoints(
	...
	if yaml_configuration[SUBDOMAIN_DISCOVERY][ALIVE_ONLY] == True:
		logger.info("[+] Alive Only Scan: True")
		sorted_subdomains_path = results_dir + '/sorted_subdomain_collection_alive.txt'
	else:
		logger.info("[+] Alive Only Scan: False")
		sorted_subdomains_path = results_dir + '/sorted_subdomain_collection.txt'
	...
	for tool in tools.split(' '):
		if tool == 'gauplus' or tool == 'waybackurls':
			if subdomain:
		...
		if tool == 'gauplus':
			...
			gauplus_command = '{} | gauplus --random-agent | grep -Eo {} | tee {}/urls_gau.txt'.format(
				input_target,
			...
			waybackurls_command = '{} | waybackurls | grep -Eo {} | tee {}/urls_waybackurls.txt'.format(
				input_target,
				valid_url_of_domain_regex,
				results_dir
			)
			...
			if subdomain:
				...
				gospider_command = 'gospider -S {}/alive.txt'.format(results_dir)
			else:
				gospider_command = 'gospider -s https://{} '.format(domain_name)

			gospider_command += ' --js -t 100 -d 2 --sitemap --robots -w -r | grep -Eo {} | cut -d "]" -f1  | tee {}/urls_gospider.txt'.format(
				valid_url_of_domain_regex,
				results_dir
			)
			...
			if subdomain:
				subdomain_url = subdomain.http_url if subdomain.http_url else 'https://' + subdomain.name
				katana_command = "katana -d 4 -jc -kf all -hl -no-sandbox -u {} -ef eot,jpeg,gif,css,tif,tiff,otf,woff2,ico,pdf,txt,js,doc,docx,ttf,woff,svg,png,jpg,mp4,xls,xlsx,ppt,pptx,bmp -silent -nc -c 250 -retry 2 -timeout 5 -o {}/urls_katana.txt".format(subdomain_url, results_dir)
			elif scan_type == 'deep' and domain:
				katana_command = "katana -d 4 -jc -kf all -hl -no-sandbox -u {0}/alive.txt -ef eot,jpeg,gif,css,tif,tiff,otf,woff2,ico,pdf,txt,js,doc,docx,ttf,woff,svg,png,jpg,mp4,xls,xlsx,ppt,pptx,bmp -silent -nc -c 250 -retry 2 -timeout 5 -o {0}/urls_katana.txt".format(results_dir)
			else:
				katana_command = "katana -d 4 -jc -kf all -hl -no-sandbox -u https://{} -ef eot,jpeg,gif,css,tif,tiff,otf,woff2,ico,pdf,txt,js,doc,docx,ttf,woff,svg,png,jpg,mp4,xls,xlsx,ppt,pptx,bmp -silent -nc -c 250 -retry 2 -timeout 5 -o {}/urls_katana.txt".format(domain_name, results_dir)
			...
			if subdomain:
				subdomain_url = subdomain.http_url if subdomain.http_url else 'https://' + subdomain.name
				hakrawler_command = "echo {} | hakrawler -subs -u | grep -Eo {} | tee {}/urls_hakrawler.txt".format(subdomain_url, valid_url_of_domain_regex, results_dir)
			elif scan_type == 'deep' and domain:
				hakrawler_command = "cat {0}/alive.txt | hakrawler -subs -u | grep -Eo {1} | tee {0}/urls_hakrawler.txt".format(results_dir, valid_url_of_domain_regex)
			else:
				hakrawler_command = "echo https://{} | hakrawler -subs -u | grep -Eo {} | tee {}/urls_hakrawler.txt".format(domain_name, valid_url_of_domain_regex, results_dir)
	...
	#os.system('cat  {0}/final_urls.txt | sort -u | python3 /usr/src/github/urless/urless.py ')
	#os.system('sort -u {0}/final_urls.txt -o {0}/{1}'.format(results_dir, output_file_name))
	logger.info('[*] runing urless on all discovered urls...')
	urless_command = ('cat {0}/final_urls.txt | sort -u | python3 /usr/src/github/urless/urless.py | tee {0}/{1}'.format(results_dir, output_file_name))
	logger.info(urless_command)
	os.system(urless_command)
	...
	httpx_command = '/go/bin/httpx -r {0} -l {1}/{2} -status-code -content-length -ip -cdn -title -tech-detect -json -follow-redirects -random-agent -retries 3 -timeout 10 -o {1}/final_httpx_urls.json'.format(dns_resolvers, results_dir, output_file_name)
...
def vulnerability_scan(
	...
	if domain:
		...
		scan_type =  yaml_configuration[VULNERABILITY_SCAN][SCAN_INTENSITY]
		logger.info('************ {} **************'.format(scan_type))
		if scan_history.scan_type.fetch_url and scan_type == 'deep':
			logger.info('Sorting all urls for deep nuclei scan')
			os.system('cat {0}/all_urls.txt | grep -Eiv "\\.(eot|jpeg|gif|css|tif|tiff|otf|woff2|ico|pdf|txt|js|doc|docx|ttf|woff|svg|png|jpg|mp4|xls|xlsx|ppt|pptx|bmp)$" | unfurl -u format %s://%d%p >> {0}/unfurl_urls.txt'.format(results_dir))
			os.system('sort -u {0}/unfurl_urls.txt -o {0}/unfurl_urls.txt'.format(results_dir))
			urls_path = '/unfurl_urls.txt'

		vulnerability_scan_input_file = results_dir + urls_path

		nuclei_command = 'nuclei -r {} -json -l {} -o {}'.format(
			dns_resolvers, vulnerability_scan_input_file, vulnerability_result_path)
	else:
		url_to_scan = subdomain.http_url if subdomain.http_url else 'https://' + subdomain.name
		nuclei_command = 'nuclei -r {} -json -u {} -o {}'.format(dns_resolvers, url_to_scan, vulnerability_result_path)
	...
	if CUSTOM_NUCLEI_TEMPLATE in yaml_configuration[
		...
		nuclei_command = nuclei_command + ' -t /root/nuclei-templates -t /root/fuzzing-templates'
