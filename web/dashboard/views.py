...
def index(request):
    ...
    endpoint_count = EndPoint.objects.all().values('http_url').distinct().count()
    scan_count = ScanHistory.objects.all().count()
    #subdomain_count = Subdomain.objects.all().count()
    subdomain_count = Subdomain.objects.all().values('name').distinct().count()
    ...
    #alive_count = \
    #    Subdomain.objects.all().exclude(http_status__exact=0).values('name').distinct().count()

    alive_count = Subdomain.objects.all().values('name').distinct().filter(ip_addresses__isnull=False).count()

    #endpoint_alive_count = \
    #    EndPoint.objects.filter(http_status__exact=200).count()

    endpoint_alive_count = \
        EndPoint.objects.filter(http_status__exact=200).values('http_url').distinct().count()
